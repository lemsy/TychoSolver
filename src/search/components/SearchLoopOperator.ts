...
export const SearchLoopOperator = async ({
    ...
    const evaluationStep: PipelineStep<any> = {
        apply: async (solution: any) =>
            await evaluationOperator.evaluate(solution)
    };

    const neighborhoodStep: PipelineStep<{ solution: any; fitness: number }> = {
        apply: async ({ solution, fitness }) =>
            await neighborhoodOperator({
                solution,
                fitness,
                neighborhoodFunction,
                objectiveFunction,
    ...
    const pipeline = new SequentialOperator<any>([
        {
            apply: async (input: { solution: any; fitness?: number }) => {
                const fitness =
                    input.fitness !== undefined
                        ? input.fitness
                        : await evaluationStep.apply(input.solution);
    ...
    while (true) {
        ...
        if (
            !next ||
            terminatedAfterMove ||
            (!useDynamic && next.solution === state.solution)
        ) {
            return {
