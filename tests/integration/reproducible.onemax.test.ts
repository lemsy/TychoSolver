...
describe('Reproducible OneMax with seed', () => {
    it('should produce same best fitness for same seed', async () => {
        ...
        const ga1 = new GeneticAlgorithm<number[]>(fitness, {
            ...config,
            initializationOperator: new SeededInit()
        });

        const best1 = await ga1.evolve();
        const fit1 = fitness(best1);

        const ga2 = new GeneticAlgorithm<number[]>(fitness, {
            ...config,
            initializationOperator: new SeededInit()
        });

        const best2 = await ga2.evolve();
        const fit2 = fitness(best2);
