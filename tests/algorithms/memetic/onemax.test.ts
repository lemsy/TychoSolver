...
describe('MemeticAlgorithm - OneMax Problem', () => {
    it('should solve the OneMax problem (maximize number of 1s)', async () => {
        ...
        const initializationOperator =
            new MemeticInitializationOperator<OneMaxGenome>(oneMaxIndividualFactory);

        const evaluationOperator =
            new GAEvaluationOperator<OneMaxGenome>(oneMaxFitness);

        const selectionOperator =
            new SelectionOperatorImpl<Individual<OneMaxGenome>>();

        const crossoverOperator =
            new CrossoverOperatorImpl<OneMaxGenome>();

        const mutationOperator =
            new MutationOperatorImpl<OneMaxGenome>((gene) => 1 - gene);
