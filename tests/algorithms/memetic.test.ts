...
import { NeighborhoodFunction } from '../../src/search/types';
...
describe('MemeticAlgorithm', () => {
    it('should return a solution for a simple numerical problem', async () => {
        // Example problem: maximize f(x) = x² for x in [0, 10]
        ...
        const crossoverOperator = {
            crossover: (a: number, b: number) =>
                [Math.round((a + b) / 2), Math.round((a + b) / 2)] as [number, number]
        };

        const mutationOperator = {
            mutate: (individual: number) =>
                Math.max(0, Math.min(10, individual + (Math.random() < 0.5 ? 1 : -1)))
        };
    ...
    it('should solve a binary optimization problem', async () => {
        ...
        const individualFactory = () =>
            Array.from({ length: GENOME_LENGTH }, () => Math.round(Math.random()));

        const initializationOperator =
            new MemeticInitializationOperator<BinaryGenome>(individualFactory);

        const countOnes = (genome: BinaryGenome) =>
        ...
        const flipOneBitNeighborhood: NeighborhoodFunction<BinaryGenome> = (
            genome: BinaryGenome
        ) => {
            ...
            for (let i = 0; i < genome.length; i++) {
                ...
                // TS7 + noUncheckedIndexedAccess
                neighborGenome[i] = 1 - neighborGenome[i]!;
        ...
        expect(countOfOnes).toBeGreaterThanOrEqual(GENOME_LENGTH * 0.9);
